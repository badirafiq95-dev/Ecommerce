...
function compactPaymentScreenshot(paymentScreenshot) {
...
export function compactOrderForStorage(order) {
  return {
    ...order,
    paymentScreenshot: compactPaymentScreenshot(order.paymentScreenshot)
  };
}
...
function orderBelongsToCustomer(order, uid = "", email = "") {
  ...
  return order.userId === uid || (!order.userId && normalizedEmail && orderEmail === normalizedEmail);
}
