...
let couponMemoryCache = process.env.NODE_ENV === "production" ? [] : DEFAULT_COUPONS;
...
export function readCoupons() {
  ...
  if (process.env.NODE_ENV === "production") return couponMemoryCache;
...
export function saveCoupons(coupons) {
  ...
  couponMemoryCache = Array.isArray(coupons) ? coupons : [];
  try {
