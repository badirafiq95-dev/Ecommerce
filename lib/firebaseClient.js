...
const LOCAL_PROFILE_KEY_PREFIX = "freaking-collectibles-profile";
...
export function readLocalCustomerProfile(uid) {
  ...
  try {
    const saved = window.localStorage.getItem(getLocalProfileKey(uid));
...
function writeLocalCustomerProfile(uid, profile) {
  ...
  try {
    ...
    // Ignore local storage failures; Firestore remains the source of truth when available.
  }
}
...
export function listenAllOrders(callback) {
  return onSnapshot(
    ...
    (error) => {
      logFirestoreWarning("all orders listener", error);
      callback([]);
    }
  );
}
